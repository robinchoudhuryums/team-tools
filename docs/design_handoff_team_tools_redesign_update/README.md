...
**STATUS: IMPLEMENTED.** Built as a scannable table via the **shared `mtRenderTable_` component** (the same one that drives the Metrics team table) — sortable headers (`cnStatsSort_`/`cnStatsSortReps_`, persisted in `CN_STATE.statsSort`), tri-tone `% Ans` + `Coverage` via the shared `mPctClass_`/`mCoverageBadge_`, columns Rep / Notes / Action / Training / Review / Emails / Median / % Ans / Coverage, rep name a drill-link to Per-Rep View (`cnStatsDrillDown_`), graceful `—` for missing CDR. This matches the original intent and reuses one table component across both modules.

*Cleanup remaining:* the old per-rep-card CSS (`.cn-stats-grid-outer`, `.cn-stats-card`, `.cn-stats-head`, `.cn-stats-name`, `.cn-stats-span`, `.cn-stats-total`, `.cn-stats-total-lbl`, `.cn-stats-grid`, `.cn-stats-row`, `.cn-stats-k`, `.cn-stats-v`) is now dead — the render path emits `mtRenderTable_` markup instead. Safe to delete; keep `.cn-stats-name-link` and `.cn-stats-foot-note` (both still used).
...
- `loaders_and_motion.md` — unified loader system (4 roles) + motion system (10 effects), with chosen variants, CSS, and call sites.
- `popups_addendum.md` — in-app pop-up fixes (toast-type normalization, `.toast-info`, overlay-entrance unification, `day-info-row` token mismatch).
- `email_styling.md` — Call Note (modernized + per-template theming) & PPD email (questionnaire table + de-flexed recommendation list + palette mapping). Company navy.
...
- `Loader Options.dc.html` — the 6 loader candidates (chosen mix in `loaders_and_motion.md`).
- `Motion Demo.dc.html` — all 10 motion effects, interactive (chosen variants in `loaders_and_motion.md`).
- `Email Templates.dc.html` — final Call Note (modernized) + PPD recommendation email (company navy).
- `Email Templates — Call Note variants.dc.html` — the four update themes (Verified Shipping / OOP / Close Order / Repeat Resupply).
