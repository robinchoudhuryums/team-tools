...
# Cycle 21post (closed 2026-09-23 — between-cycles operator work T1–T10 on the
# Reference lookups and area eligibility. T1–T9 DEPLOYED; T10 merged with this
# close-out and awaits its deploy. Reflected net +8.)

**Close-out note.** Ten batches (T1–T10) followed cycle 21's deploy, the 19pre
pattern: operator-requested work on the Reference lookups, none of it
scan-derived. Merged as PRs #264–#268 and the close-out PR; T1–T6 deployed
2026-09-22, T7–T9 2026-09-23. The defining event was the first T1–T6 deploy
exposing an operator `LocationAcceptance` tab the reader could not read
(headers in row 2, no `Name` column): the registry came back empty with NO
error, every radius verdict blamed the PRICING sheet, and the operator rewrote
correct cells to match — g142, and the High fix in T7. Reflected 2026-09-23 as
**8 − 0 = 8** (`.cycle/blocks/21post-a-reflect.md`; metrics row `21post`),
after three corrections to the self-reported 11, all one way: a real CLASS of
defect scored as if it had an occurrence. Estimates: all ten recorded before
the first edit — 36.5 h vs ~26.5 h, 0.73x. **At close-out NOTHING in the
series has been WALKED** (S64, S18, S112, S73), and T10 is undeployed; both
carry into the fresh STATE with every standing debt below.
...
# Cycle 21 (closed 2026-09-21 — batch R DEPLOYED and operator-confirmed;
# batch S needed no deploy. Reflected net +1.)

**Close-out note.** The deploy of batch R on 2026-09-21 surfaced THREE defects
in the Reference lookups, all of which predate this cycle: the drawer laid its
lookup sections out in a row because both were wrapped in `.kbd-sec` (a
mono/uppercase flex HEADING bar) since `0743f07`; `.kb-ins-row` had never
carried a CSS rule at all, so price and eligibility results ran together while
the insurance results beside them were carded; and the item name shared one div
with every price, so a three-price item read as a sentence. They are fixed in
**PR #264** (green, open at close-out) together with the three things that would
have caught them — a DOM pin that each control sits in `.kbd-lookup` and not
`.kbd-sec`, fixtures for the three lookup RPCs, and five scenarios that type a
query and shoot the RESULTS. None of that is cycle-21 scope; it is recorded here
because the cycle's deploy is what exposed it.

**Why they survived:** no visual scenario had ever opened the KB drawer, the
mock had no fixture for any lookup endpoint so the matrix could only photograph
empty boxes, and the DOM pins asserted the drawer MOUNTS each section — which it
did, inside a heading bar. A mount check cannot see WHERE something mounted.
...
## Cycle 20 (closed 2026-09-18 — DEPLOYED and CONFIRMED: New version cut, `runAllTestsPartB` 122/122 on the deployed project, S112 walked and working)

_Moved verbatim from `.cycle/STATE.md` per the close-out procedure, on deploy confirmation rather than on a new audit opening. The `/broad-scan` of 2026-09-17 found 53 issues; 52 are implemented and live, one is deferred to the operator (F-09's holiday fallback). Cycle 20 is fully reflected (`.cycle/blocks/20-a-reflect.md`, net 28 − 3 = 25 — read THAT for the tally, not the seven implementation blocks, which sum to 29 and which the reflection corrects in four places). Two post-merge rounds rode on the end of this cycle and are recorded in its block: the accrual reconcile test's ledger baseline (PR #261, found by the first full editor run ever to reach the integration tier) and `bite.sh --fn` with the doc pass for both (PR #262)._
...
decision). **DEPLOYED AND CONFIRMED 2026-09-18** — `clasp push -f` plus a New
version were cut, `runAllTestsPartB` read 122/122 on the deployed project, and
S112 (OOP pricing, the composer price picker and area eligibility both ways)
was walked and works.
Scope: broad
Test Command: manual
...
**Deploy CONFIRMED 2026-09-18.** `clasp push -f` plus a New version were cut;
`runAllTestsPartB` read 122/122 on the deployed project after PR #261's test
fix; S112 (OOP pricing + area eligibility) was walked by the operator and
works. All 52 implemented findings of cycle 20 are live. This block is
therefore archived here per the close-out procedure, and STATE.md is reset.
...
## Cycle 19 (closed in substance 2026-09-17 — deployed, smoke green, /reflect OWED; archived 2026-09-17 when the /broad-scan opened cycle 20)

_Moved verbatim from `.cycle/STATE.md` per the close-out procedure. Cycle 19's `/reflect` had NOT run when this block landed — its four reflections (19-a..19-d) cover everything up to Batch T/R; the OOP-A/B/C, ELIG, SP/SP2, PTO and H1–H3 rounds are reflected NOWHERE yet. Read this block, not STATE.md, for that reflection._
...
## Completed this cycle
- H1/H2/H3 (2026-09-17, cross-repo, branch `claude/inspiring-volta-7u9szs`, NOT yet merged) | 10_core.js, 20_timeclock.js, 40_metrics.js, 00_config.js, metrics/tc partials, Tests.js, run.js | the CDR Report's `Company Holidays` tab is the ONE calendar (g123/INV-210); Answer % is the dashboard's formula against its PUBLISHED `Dashboard Standards` (g124/INV-211, `CDR_ALERT_THRESHOLD` removed); span-bounded DQE reads + Sheets-serial dates (g125/INV-212); MAIL-SWEEP pin, `.gitattributes`, CI TZ pin. Block: `.cycle/blocks/20pre-H3-bridge-infra-broad-implement.md`. Post-merge: deploy + `runSmokeTests` (Expected now prints 336); set `CDR_DASHBOARD_DEPT` only if the dashboard's CSR header is not literally `CSR`.
...
**OOP-C (2026-09-16 late) landed after the round merged** — the operator pasted
their real `OopPricing` header row and the reader did not work against it. Fixed,
...
## Cycle 18 (closed 2026-08-21; archived 2026-09-09 when cycle 19 opened)
Moved verbatim from STATE.md at the cycle-19 /broad-scan, per the close-out
procedure in CLAUDE.md "Cycle State & Memory".
...
the Admin editor's Type column), the recording-less fallback
(`qaCreateManualRecording`, `manual-` ids), the `qaLog` tab; pure 762/0,
DOM 107/0, 11/11 bites, five `qa-log-*` scenarios shot clean. Block:
`.cycle/blocks/19pre-qa-log-broad-implement.md`. The operator will define the
rubric in the Admin editor later. NOTE: this file's stale duplicate tail
(a 2026-09-01 copy appended below the current Where-I-left-off, repeating
every heading — the exact hazard the editing rule names) was removed today.

PREVIOUS (2026-09-03). Branch `claude/ums-team-tools-design-r8ar3o`, rebased on
...
<!-- ===== cycle 17 (closed) — moved from STATE.md 2026-08-21, at cycle-18 open ===== -->
...
# Cycle 16 — Broad Scan (closed 2026-08-03)

Archived from STATE.md when cycle 17 opened (2026-08-05). Reflected: net 7
(8 production fixes − 1 new failure mode). Reflect block:
`.cycle/blocks/16-a-reflect.md` (trust it over the implementation blocks).
...
# Cycle 15 — Seams & Invariants (closed 2026-07-31)

Archived from STATE.md when cycle 16 opened. Reflected: net 2 - 1 = 1.
Reflect block: `.cycle/blocks/15-a-reflect.md` (trust it over the
implementation block — it corrected the tally in both directions).
