...
- **Two price columns can hold the same figure** (`OOP Price` and `Pick-Up Cost` are both $920 here), so the picker shows two Inserts for one number under different labels. Honest, and truthfully what the sheet says, but worth an operator eye on whether one of those columns is redundant.
