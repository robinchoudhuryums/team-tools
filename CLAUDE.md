...
- **"Not found" and "could not look up" are DIFFERENT answers — the geocoder's quota, denial and throw all collapsed to null, and every caller told the rep the ADDRESS was wrong (Batch 2, 2026-09-17). And a reader that names its failure BESIDE an empty result needs one refusing helper at its callers: `cdrAgentsOrThrow_` (cycle 22 M7).** Fires when a lookup's miss and its failure share one return value. Verify: the F-15 geocoder pin + the M7 pin. [Detail](docs/gotchas.md#g128-not-found-and-could-not-look-up)
...
| Invariant library entries | 246 | `.cycle/config.md` |
