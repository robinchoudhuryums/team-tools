...
- **Multi-tool registry with tab sub-navigation.** The `TOOLS` object
  ...
  **The admin tier is a CLIENT-VISIBILITY distinction only right now** — the
  underlying config endpoints still gate on `isManager` server-side (admin ⊆
  manager, so there's zero security delta today). Server-side `assertAdminCaller_`
  for the config endpoints is a follow-on, only material once a non-admin manager
  exists. The **Manage module** (`manage` tool, sidebar-last) is the consolidated
...
- **Script Property `ADMIN_EMAILS`** (optional) — comma-separated list of the
  ...
  keeps Admin access). No redeploy needed to change it. NOTE: this gates Admin
  CLIENT-side; the underlying config endpoints still gate on `isManager`
  server-side (a follow-on would add `assertAdminCaller_`).
