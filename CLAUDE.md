...
- **Manager-only operations check `callerEmp.isManager`.** Any new
  ...
  privilege escalation. **EXCEPTION — the admin tier (INV-136):** the 30
  Admin-exclusive endpoints (26 Manage-module Admin-tab config/system
  endpoints + the Reference content-authoring set
  `kbSaveItem`/`kbDeleteItem`/`kbUploadImage`/`kbConvertDriveDoc` — the
  authoritative list is in INV-136; cycle-11 doc fix: earlier copies of this
  count said 24/28)
...
- **Multi-tool registry with tab sub-navigation.** The `TOOLS` object
  ...
  **The admin tier is enforced BOTH client-side (the `adminOnly` tab) AND
  server-side** — the 30 Admin-exclusive endpoints (INV-136's list) gate on
...
- **Script Property `ADMIN_EMAILS`** (optional) — comma-separated list of the
  ...
  CLIENT-side AND the 30 Admin-exclusive endpoints SERVER-side (`emp.isAdmin`,
...
`CSR_TRANSFER_EXPECTED_HEADERS` ↔ `CSRT` behavioral pin — pure now 330,
DOM 65. Editor suite
...
rejectsBadDate horizon cases). Use
