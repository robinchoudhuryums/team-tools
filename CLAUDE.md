...
- **Cycle 17 (top-5 + batches ②–⑤) adds NO new operator state** — no Script
  ...
  `status` on search hits), so a not-yet-redeployed client renders as before.
  ...
  truncate at 1000 chars. **Post-deploy: run `runAllTests()`** — incl. the
  not-yet-executed `test_updateTimeOff_mixedCaseStatusCell` follow-on if
