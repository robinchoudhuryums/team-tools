...
- **`CN.DATE_LOCAL` is a Sheets-coerced Date on read — and until 2026-09-18 only the wrong-HELPER scan (PTA-3) covered it, never a RAW read, on the most recovery-guarded column in the codebase.** Fires when you read `CN.DATE_LOCAL`. Verify: the INV-142 global scan (now incl. `DATE_LOCAL`) + the F1 derived half. [Detail](docs/gotchas.md#g16-cn-date-local-is-a-sheets-coerced)
...
- **Two readers of ONE operator sheet share ONE column resolver, ONE scorer, ONE payload shape and ONE renderer — the send-time verifier keyed column A while the picker resolved by header, so every quoted send was refused for a listed item (Batch 1, 2026-09-17); then, with both unified, the eligibility endpoint still hand-picked `prices[0]` and drew it bare, so the DELIVERY surface quoted the pick-up price (R, 2026-09-18 — live for a day).** Fires when a second function reads an operator-maintained tab another function already reads by header — and again when it picks its own subset of the fields, or draws them with its own renderer. Verify: the F-04 shared-scorer pin + the OOP-B real-shape grid + R-1 (the field list DERIVED from `oopRowObj_`) + the R DOM three-price drive + the K7 pin (the verifier keyed ONE row per name, so a duplicate-name row's quote was refused — cycle 22). [Detail](docs/gotchas.md#g126-two-readers-of-one-operator-sheet)
...
- **ScriptLock around every mutating op — and a helper that runs INSIDE a locked job's catch takes a different lock, never re-takes and releases the job's (cycle 22 A8).** Fires when you add a server function that writes to a sheet, or a stamp called from a trigger's error path. [Detail](docs/gotchas.md#g17-scriptlock-around-every-mutating-op)
...
- **A diagnostic that can never be clean is worse than none — the CDR name-match card (cycle 15); a SECOND instance 2026-09-18, the queue panel's `truncated` computed from the tab's length rather than the window's, so it warned on every run for ever.** Fires when you tone a health card off a count, or set a caution flag from something that is permanently true in production. [Detail](docs/gotchas.md#g02-a-diagnostic-that-can-never-be-clean)
...
- **An operator-maintained data source that a DECISION ENGINE reads needs a shape check, and the fail direction on unreadable data must be CHOSEN (F9, cycle-16 — FIXED) — and a parser that REMOVES what it understood fails on what is left, never drops it: the eligibility grammar read the first of two distances and ignored "except Hawaii" (cycle 22 K1 + K3).** Fires when an engine reads operator-maintained data it cannot parse, or strips the parts it recognises. Verify: the K1 + K3 pins. [Detail](docs/gotchas.md#g41-an-operator-maintained-data-source-that-a)
...
- **A best-effort overlay whose ABSENCE is reassuring must announce itself (F4, cycle-16 — FIXED); a failed tag load read "No tags in use yet" and a failed ClientErrors read "no client errors" (cycle 22 A4 + A6).** Fires when you wrap an overlay read in a bare `catch`. [Detail](docs/gotchas.md#g53-a-best-effort-overlay-whose-absence-is)
...
- **A per-day SERIES must tell "no data" apart from a real zero — `|| 0` against a sparse map invented a measurement, and the manager sparkline read "0 hours worked" for a rep who was still clocked in (Batch 7, 2026-09-18). A second instance: the Metrics trends drew a PTO day as zero calls (cycle 22 M9).** Fires when you fill a fixed-length series from a keyed map, or default a missing datum for a chart. Verify: the F-48 three-state assertion + the `renderEmpSparkline_` drive + the fixture's own null day + the M9 sweep. [Detail](docs/gotchas.md#g136-a-per-day-series-must-tell-no-data)
- **A computed ZERO that could mean three different things must say WHICH — the accrual audit row (operator 2026-09-14). An EMPTY set has no median either: the coaching board read "median 0 days" for no acknowledgements (cycle 22 D2).** Fires when you record or render a zero a reader could reach by more than one route. Verify: the `previewPtoAccruals` pin + the D2 pins. [Detail](docs/gotchas.md#g114-a-zero-that-could-mean-three-different)
- **A CONFIG seed that is a PLAUSIBLE substitute for real config is worse than no seed — the warehouse registry's bare city names geocoded to city CENTRES (ELIG, shipped and removed 2026-09-16).** A FALLBACK is the same shape: a configured Dept Requests store that would not open fell back to the payroll sheet (cycle 22 A3). Fires when you seed a default or write a fallback — ask what it produces when nobody replaces it, and whether the store was UNSET or merely unreachable. Verify: the NO-seed pin + the A3 pin. [Detail](docs/gotchas.md#g122-a-config-seed-that-is-a)
- **"Not found" and "could not look up" are DIFFERENT answers — the geocoder's quota, denial and throw all collapsed to null, and every caller told the rep the ADDRESS was wrong (Batch 2, 2026-09-17). And a reader that names its failure BESIDE an empty result needs one refusing helper at its callers: `cdrAgentsOrThrow_` (cycle 22 M7). "Partly found" is a third answer — a partial geocode is refused, never measured (cycle 22 K5).** Fires when a lookup's miss and its failure share one return value. Verify: the F-15 geocoder pin + the M7 pin + the K5 pins. [Detail](docs/gotchas.md#g128-not-found-and-could-not-look-up)
...
- **The holiday calendar is the CDR Report's `Company Holidays` tab, read through ONE accessor (`getCompanyHolidays_`) that REPLACES the federal list rather than adding to it (H1, 2026-09-17) — and since Batch 3 the calendar that is LIVE is stated on the Admin → System CDR row (`cdrHolidayProbe_`), so the fallback is never silent. The Dashboard's Yesterday card was a third calendar-yesterday reader until cycle 22 M2.** Fires when you compute a "previous workday", walk business days, or read `getUsHolidays_` directly. Verify: the H1-1..H1-5 pins + `test_companyHolidays_*` + the F-08/F-09 pins. [Detail](docs/gotchas.md#g123-the-holiday-calendar-is-the-cdr-report-s)
- **Answer % is the Department Dashboard's formula (`answered / (answered + missed)`), and the target, amber band and team-average exclusions come from the dashboard's published `Dashboard Standards` tab -- a null standard renders NO tone, line or badge, never a hand-carried number (H2, 2026-09-17) — the standard's SOURCE is rendered beside the target, a NULL band is `mtAnswerBand_`'s rule (no amber tier) on every surface, no answered + missed is a NULL rate, and the badge judges the previous WORKDAY (Batch 3); a Team Metrics rep with no calls is a NULL rate on the row too (cycle 22 M1).** Fires when you compute or tone an answer rate, add a KPI band, or read `CONFIG` for a threshold. Verify: the H2-1..H2-4 pins + the F-07/F-08/F-32/F-35 pins + the M1 pin + `test_cdrAnswerPct_isTheDashboardFormula` / `test_dashboardStandard_readsFixtureTab`. [Detail](docs/gotchas.md#g124-answer-is-the-dashboard-s-formula-and)
...
- **A NULL a payload ships is a RULE one helper decides, never a per-consumer default — and a caption that names a published number never carries a literal of it (Batch 3, 2026-09-17).** Fires when two surfaces read the same shipped field, or a client string interpolates a server number with a `|| N`. Verify: the F-07 grid pin + the F-08 `threshold || \d` ban. [Detail](docs/gotchas.md#g131-a-null-a-payload-ships-is-a-rule)
- **A period-to-date window over a LAGGED source counts days of DATA, not of calendar — the MTD comparison took days 1..d of last month against d-1 days of calls, and the run-rate divided by a today that never has data (cycle 22 M8, 2026-09-23).** Fires when you compare a to-date window with an earlier one, or project one forward, over a source that lags. Verify: the `dashboardPrevRange_` / `dashboardPeriodRange_` pins + the M2/M8 endpoint pin. [Detail](docs/gotchas.md#g148-a-lagged-source-s-windows-count-days)
...
- **A static net that PRE-DECLARES names the runtime does not provide silences the class it exists to catch — six ADVANCED services sat in the linter's globals list while the manifest enabled none, so a use of any would have linted clean and thrown at its first call (Batch 7, 2026-09-18).** Fires when you add a name to an allowlist to make a linter quiet. Verify: the F-50 pin drives the derivation over manifests the repo does not have. [Detail](docs/gotchas.md#g137-a-static-net-that-pre-declares-names)
- **A pin whose NAME promises a behaviour must DRIVE it, and an assertion that cannot be made to fail is DELETED rather than dressed up (Batch 7, 2026-09-18).** Fires when you write a pin over a drivable function, and every time you read a NO BITE. Verify: the three rewritten pins + the width pin that says why it cannot be driven. [Detail](docs/gotchas.md#g138-a-pin-whose-name-promises-a-behaviour)
...
- **A client RPC with no mock fixture is invisible to the visual matrix, and the matrix is not in CI — T8's diagnostics panel was photographed as "no fixture" in all 16 Admin scenarios for a cycle (cycle 22 X1, 2026-09-23).** Fires when you add a `google.script.run` call, or a fixture. Verify: the two X1 pins (the shrink-only net that drives the real mock, and the fixture mirrored to the real resolvers). [Detail](docs/gotchas.md#g150-a-client-rpc-with-no-mock-fixture)
...
- [Price and area eligibility are ONE panel, because they are one question asked of one table (operator 2026-09-18)](docs/design-decisions.md#price-and-area-eligibility-are-one-panel-because-they-are)
...
admin **Manage → Admin → System → Storage Health** panel (`getStorageHealth`)
...
|---|---|---|
| Pure harness tests | 1031 | `node test/client/run.js` |
