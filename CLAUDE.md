...
- **Trigger-handler endpoints are reachable via `google.script.run`.**
  ...
  (the flag-gated consolidated morning brief, INV-151) and
  `archiveOldTimesheetRows` (the Timesheet cold-archive, INV-153) — are top-level (required: Apps Script
...
- **Intake PPD controls are engine-safe via CANONICAL-ENGLISH VALUES, not
  ...
  comma-joined; `Left (Above Knee)`… with no stray `no`), and Q38 (weight) is
  `numunit` (engine strips non-digits) — MORE reliable than the old free-text (no
...
- **Automation Health panel (Admin tab).** Manager-only, read-only
  ...
  **Detector liveness (Turn C, cycle 7):** the panel also renders a "Detector
  ...
  tripwire pins the compute→return→digest wiring + the five check keys.
...
|-------|----------------------------|------|-------|-----------|----------|
| Time Clock / ADP | `ADP_SS_ID` (CONFIG placeholder) | Employees (roster), Timesheet, TimesheetArchive (cold tier, INV-153), TimeOffRequests, AuditLog, PunchAdjustRequests, ClientErrors (INV-150) | Payroll + shared audit | kept (archive moves, never deletes) | `getAdpSS_` |
...
- **Daily automation triggers** must be installed by a manager
  ...
  installer now wires fifteen triggers:
    ...
    - `archiveOldTimesheetRows` (Timesheet cold-archive, daily manager-tz 1am — MOVES Timesheet rows older than `TIMESHEET_ARCHIVE_DAYS` to a `TimesheetArchive` tab in the same ADP spreadsheet; NEVER deletes (payroll is keep-forever — no purge tier exists for it); sub-floor windows clamp UP to `TIMESHEET_ARCHIVE_MIN_DAYS` (120); no-ops while the window is 0 (the default). INV-153)
  The install + remove TARGETS arrays both list all fifteen, so re-running
...
- **`Employees` sheet column O = `Schedule`** (Turn D, cycle 7) — an OPTIONAL
  ...
  `9:15-17:45`; bare hours like `9-17` also parse). Blank = the per-timezone
...
- **Timesheet cold-archive is OFF by default (INV-153).** `archiveOldTimesheetRows`
  (daily manager-tz 1am trigger) MOVES Timesheet rows whose date is older than
...
INV-142 | Every read of the CN `Timestamp` column routes through `cnTimestampString_` (recovers a locale-coerced Date to the as-written `yyyy-MM-dd'T'HH:mm:ss` digits in the ADP sheet tz — valid because per-rep sheets are pinned to it, INV-110/141; strings pass through). Readers: `callNoteRowToObject_` (sorting, shift-span regex, EOD display), `deleteCallNote` (the 5-min window FAIL-OPENED on a coercing sheet), `getCallNotesAmbient` (stale-flag counter), `getMyTrainingQA` (sort key). `getMyNoteHourBuckets`/`reconcileCallNotes` keep their equivalent inline guards. Pinned by the INV-142 source tripwire (all four readers must reference the helper) | Subsystem: Server
INV-143 | `sanitizeCallNotePayload_` WHITELISTS client-supplied `subformData` keys at submit: only `trainingQuestion` (trimmed, ≤2000 chars) and `completionSeconds` (finite positive number, rounded) survive; a lone legacy `flagType:'urgent'` folds into `flags:['urgent']`. Everything else (`trainingReply*`, `feedback[]`, `pinned`/`pinnedAt`, `formSubmission`, `externalEmails`, …) is stripped — those keys are written ONLY by their own gated server endpoints, keeping INV-49/50 server-enforced (a crafted submit could previously forge a manager reply or bypass the 3-pin cap). A new client-sent submit key MUST be added to the whitelist or it is silently dropped. Pinned by the sanitizeCallNotePayload_ Node tests | Subsystem: Server
...
INV-145 | The department composer sets `composer.sending` for the duration of the send RPC: `cnComposerSend_` double-fire-guards on it, and `cnCloseComposerModal_` REFUSES to close (toast, overlay stays) until the RPC settles — an Esc/backdrop-click mid-send previously ran the Save&Compose rollback and deleted the note the in-flight email references. Both RPC handlers clear the flag before any close. Pinned by the M-9 DOM test (close-refusal mid-send, normal close after settle) | Subsystem: Client (Call Notes views)
