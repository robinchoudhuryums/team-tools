...
- **Manager-only operations check `callerEmp.isManager`.** Any new
  ...
  nearly invited a gate regression),
...
- **Trigger-handler endpoints are reachable via `google.script.run`.**
  ...
  `archiveOldTimesheetRows` (the Timesheet cold-archive, INV-153) and
  `runNightlySelfTest` (the daily in-project self-test — smoke on prod,
  full suite on the dev instance; INV-162) — are top-level (required: Apps Script
...
- **Call Notes Sheet enrollment — one-click auto-provision (or manual).**
  ...
  **The "is this rep enrolled?" predicate is column L non-empty-AFTER-TRIM on
  the ENROLLMENT surfaces — but NOT yet everywhere (cycle-9 L-11, corrected
  cycle 12 / F14):** `getEnrolledCallNotesReps` (the Team Notes per-rep picker
  + the audit-panel rep filter) trims like `getCallNotesEnrollment` and
  `provisionCallNotesSheet`'s no-clobber check already did — a whitespace-only
  cell used to show the rep as enrolled in the picker (whose reads then fail on
  the blank id) while the Admin panel offered to provision them. **The earlier
  claim that this holds "everywhere" was FALSE and is the live gap F14
  describes:** 10 read sites trim (the employee-object builders at
  `getEmployeeInfo_`/`lookupEmployeeById_`, the enrollment endpoints, the
  digests/archive/purge walks) while **11 cross-rep walks still test raw
  truthiness** (`if (!sheetId) continue;` — tag taxonomy, unresolved-action
  count, shift stats, cross-rep search, export, `getStorageHealth`, the
  tag-transform walk). With a whitespace-only column L the two groups
  DISAGREE: the trimmed group correctly reads "not enrolled" (their own panel
  shows the enrollment splash), while the untrimmed group calls
  `openById(' ')`, throws into the per-rep try/catch, and **silently omits the
  rep from the aggregate** — or, in Storage Health, reports a false
  "unreachable per-rep Sheet". Durable fix (not yet applied): ONE
  `cnEnrolledSheetId_(row)` predicate + a tripwire banning raw
  `EMP.CALL_NOTES_SHEET_ID` truthiness outside it — the INV-142 / INV-154
  boundary pattern.
...
- **OPEN (cycle-12 visual audit, NOT fixed) — the four `-deep` semantic tokens
  are HUE-SHIFTED in light mode.** `styles_design_tokens.html` declares correct
  fallback hexes (`--warning-deep: #8a4500` amber, `--danger-deep: #8a1f1f`,
  `--success-deep: #0b6e40`) and then an `@supports` block replaces them with
  `color-mix(in oklch, <semantic>, var(--ink) 45%)`. **oklch interpolates hue on
  the polar arc**, and `--ink` (`#0f1623`) sits at hue ≈264, so amber (h 70)
  travels 70→0→264 — *through red*. Measured in Chromium: `--warning-deep`
  resolves to **hue 354.8 (RED)**, `--danger-deep` to 330 (magenta),
  `--success-deep` to 204 (cyan); `--info-deep` is correct only because blue
  already matches `--ink`'s hue. Dark mode mixes 25% against `--paper-card` and
  stays correct, so **the same token is a different hue family per theme**.
  254 consumers; concretely, `.actions .sec.act-lunchout` paints Lunch Out in a
  destructive red in the default theme. **Reading the token file does not reveal
  this** (the fallbacks look right) and the WCAG tripwire measures LUMINANCE,
  which a hue shift leaves untouched. Verified fix: **`in oklab`** (rectangular,
  no hue arc) reproduces the documented intent for all four. Don't add a new
  `-deep` alias, or "correct" a fallback hex, without re-deriving this.
- **OPEN (cycle-12 visual audit, NOT fixed) — `.ampm` paints a theme-flipping
  token on the clock card's FIXED gradient.** `styles.html`'s
  `.hero .clk-time .ampm { color: var(--muted) }` has specificity (0,3,0) and so
  beats `tc/script_clock.html`'s (0,2,0) `.clk-sky .clk-time { color: #fff }`.
  Every other element on that card is correctly hardcoded white; the AM/PM span
  alone flips with the theme while its background does not. Measured contrast on
  the fixed sky gradient: light 2.08:1 (blue end) / 3.46 (mid) / 5.82 (amber
  end); **dark 1.20–2.00:1 across the whole card** — AM/PM is illegible in dark
  mode on the live clock of a time-tracking app. Fix is one rule
  (`.clk-sky .clk-time .ampm`). Any NEW text on a fixed-palette surface must use
  a fixed colour, not a `--muted`/`--ink` token.
...
- **Client-side persistence is localStorage-based.** See the
  ...
  The dashboard-feedback batch REMOVED `umsDashboardCompact` — net 14;
  an earlier copy of this count said fifteen, corrected in cycle 11.)
...
- **Department emails and state tax rates are editable via the Admin
  ...
  dropped — that state has no tax rate until re-saved).
...
and `SELF_TEST_LAST_RESULT` (INV-162 — the nightly self-test outcome
