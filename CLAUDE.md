...
- **Call Notes Sheet enrollment — one-click auto-provision (or manual).**
  ...
  **The "is this rep enrolled?" predicate is column L NON-EMPTY-AFTER-TRIM
  everywhere (cycle-9 L-11):** `getEnrolledCallNotesReps` (the Team Notes
  per-rep picker + the audit-panel rep filter) now trims like
  `getCallNotesEnrollment` and `provisionCallNotesSheet`'s no-clobber check
  already did — a whitespace-only cell used to show the rep as enrolled in
  the picker (whose reads then fail on the blank id) while the Admin panel
  offered to provision them.
...
- **Smoke vs. integration tests.** `runSmokeTests()` is safe to run
  ...
  **KB tests run against a fixture too (cycle-10 M-9):** `_withTestKb_`
  ...
  is factory-pinned (tz+locale) like the CN fixture. NOTE
  `test_training_quizFlow` still writes Quizzes/QuizAttempts rows to the
  live KB store (swept by cleanup; wrap-candidate).
...
and the nightly self-test wiring pin — pure now 319, DOM 65. Editor suite
+6 more (sheet-doctor flow, legacy-hash dual-verify, self-test gate + 3
omnibus gate cases) ≈ 297. Use
