...
- **Trigger-handler endpoints are reachable via `google.script.run`.**
  ...
  non-destructive nightly Sheets back-fill) and `sendAutomationHealthDigest`
  (the daily automation-FAILURE push) — are top-level (required: Apps Script
...
- **Daily automation triggers** must be installed by a manager
  ...
  installer now wires twelve triggers:
