...
- **Call Notes ambient polling stops on tool switch.** `showView()`
  ...
  `cnStartAmbientPolling_()` restarts it — but its only callers today are
  the LOG tab's enter path, so landing directly on another CN tab
  (History / Search / Team Notes / Admin via deep-link or refresh-restore)
  leaves the poller off until the rep visits Log once (cycle-8 backlog
  item — the badge + ≤60s flag propagation are Log-gated in practice).
