  ...
  registry reorg; keep it in step with `Object.keys(TOOLS).length`:
   ...
   - **Intake** — patient-intake forms ported from the bound
     ...
     **Five** tabs: **PPD** (Patient Profile &
     ...
     design tokens. **The 2026-08-25 operator round added four form
   ...
   - **QA** — call-recording review (Phase 1, operator 2026-08-27).
     ...
     **`qaStats`** (per-agent table via `mtRenderTable_` — recordings /
     ...
     unshared) feeding a read-only, UNGATED **My Reviews** tab
     (`qaMyReviews` → `getMyQaReviews`, EMPLOYEE-gated — the tab that
     makes the QA tool visible to every rep via `toolVisibleForUser_`;
     doubly scoped: SharedMs set AND Agent = the caller's roster name;
     latest scorecards + ACTIVE comments, no audio path — playback stays
     behind the `canSeeQa_` Drive boundary, a follow-on);
     **coverage-fair sampling** ("Sample 3 for me" — `qaSampleRecordings`
     ...
     widest per-criterion gap, FACTS ONLY per the Coverage rule). See
     INV-196.
...
- **Manager-only operations check `callerEmp.isManager`.** Any new
  ...
  privilege escalation. **EXCEPTION — the admin tier (INV-136):** the **48**
...
- **The 2026-08-27 break-schedule editor + QA Phase 2 round adds NO operator
  ...
  **Agent** field (datalist of roster names — free text allowed for
  ...
  QA tool in their sidebar with ONE read-only tab, My Reviews** — empty
  ("No reviews have been shared with you yet") until a reviewer explicitly
  releases a review; nothing is shared automatically, and agents still
  cannot reach the queue, playback, or stats; (f) the recording detail's
...
- **The QA module Phase 1 (operator 2026-08-27) needs THREE Script Properties
  ...
  managers only. Agents saw nothing in v1; since Phase 3 they get only the
  read-only My Reviews tab (empty until a review is SHARED). Workflow: drop
  ...
  `runAllTests()`** — expect **295** (the new `qa_gates_rejectNonMember`
...
- **The 2026-08-27 correction rounds (sender identity, brand sweep, KB editor
  ...
  before. Post-deploy `runAllTests()` now expects 294 (the new
...
into a standalone `page.html`, and `node shoot.mjs` renders a 44-scenario
...
Test Suite:
  web-app/Tests.js, test/client/harness.js, test/client/run.js, test/client/dom/boot.js, test/client/dom/runDom.js, test/visual/build.mjs, test/visual/mock.js, test/visual/shoot.mjs, test/visual/a11y-names.mjs, test/visual/print-check.mjs, test/visual/a13-measure.mjs, test/visual/map-check.mjs, test/visual/settings-check.mjs, test/visual/package.json, .github/workflows/client-tests.yml, scripts/cycle-context.mjs, package.json
...
S90 | QA module — sync, queue, playback, timestamped comments | Subsystem: Server, Client (QA views)
  Steps:
    ...
    - Confirm the queue cards show status/assignee pills + comment counts; Claim one card
      ...
      refused; as a manager, Assign… validates against the QA_MEMBERS list
    ...
    - **(Phase 3)** On a scored recording with NO agent set, click **Share with
      ...
      rep's login, open QA → **My Reviews** → the recording lists the scorecard +
      timestamped comments, read-only, with no play button anywhere; as a DIFFERENT
      rep, confirm My Reviews stays empty (name scope); Unshare → the review leaves
      the agent's tab on next load
    - **(Phase 3)** As a rep with no shared reviews, confirm the QA tool now appears
      in the sidebar with ONLY the My Reviews tab (queue/stats absent), and
      `?tool=qaQueue` bounces to a visible tab
