...
- **Roster cache invalidation + key bump.** Employee data is cached
  for 300s under `ROSTER_CACHE_KEY` (currently `employee_roster_v7`).
...
- **`ROSTER_CACHE_KEY` = `'employee_roster_v7'`** — bumped when the
  `Departments` column (N) landed for DeptRequests v2 (previously v6 for
  `ManagerEmail`/T3, v5 for CallNotesSheetId). After deploying, stale v6
...
INV-28 | Whenever the `EMP` enum gains or changes columns, `ROSTER_CACHE_KEY` is bumped (currently `employee_roster_v7`) so old cached entries with the wrong column shape are not served | Subsystem: Server
