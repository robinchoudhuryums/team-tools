...
- **A per-day SERIES must tell "no data" apart from a real zero — `|| 0` against a sparse map invented a measurement, and the manager sparkline read "0 hours worked" for a rep who was still clocked in (Batch 7, 2026-09-18).** Fires when you fill a fixed-length series from a keyed map, or default a missing datum for a chart. Verify: the F-48 three-state assertion + the `renderEmpSparkline_` drive + the fixture's own null day. [Detail](docs/gotchas.md#g136-a-per-day-series-must-tell-no-data)
...
|---|---|---|
| Pure harness tests | 917 | `node test/client/run.js` |
| DOM harness tests | 132 | `node test/client/dom/runDom.js` |
