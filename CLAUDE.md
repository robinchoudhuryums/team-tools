...
- **web-app/** — Multi-module browser web app deployed at one Web App
  ...
  registry reorg; keep it in step with `Object.keys(TOOLS).length`:
   ...
   - **QA** — call-recording review (Phase 1, operator 2026-08-27).
     ...
     `'QA access required.'`); **agents do not see their reviews in v1**
     (operator decision), so `toolVisibleForUser_` hides the whole tool
     from them. Ingestion (operator decision): the operator DROPS
     ...
     **`qaStats`** (per-agent table via `mtRenderTable_` — recordings /
     ...
     missing average reads as an em dash, never 0). Phase 3 (sampling,
     calibration, agent-facing reviews) is scoped but unbuilt. See
     INV-196.
...
- **The 2026-08-27 break-schedule editor + QA Phase 2 round adds NO operator
  ...
  **Agent** field (datalist of roster names — free text allowed for
  ...
  deployed). **Post-deploy: run `runAllTests()`** — still **295** (the
  omnibus + QA gate cases grew IN PLACE: `saveBreakSchedules` joined the
  admin set, and the QA case now covers 12 endpoints).
- **The QA module Phase 1 (operator 2026-08-27) needs THREE Script Properties
  ...
  managers only. Agents never see it (v1 operator decision). Workflow: drop
...
S90 | QA module — sync, queue, playback, timestamped comments | Subsystem: Server, Client (QA views)
  Steps:
    ...
    - In the QA spreadsheet's shared AuditLog rows (`QaSync`/`QaStatusChange`/`QaAssign`/
      `QaCommentAdd`/`QaAgentSet`/`QaScorecardSave` on the ADP sheet's AuditLog tab),
      confirm counts/ids ONLY — no file names, no comment text, no agent names, no
      ratings
  ...
  truncation is reported, never silent. Pinned by QA-1..QA-6 + `test_qa_gates_rejectNonMember`;
  the player itself is manual-only (no harness can drive `<audio>`).
