...
- **Fifteen client-side localStorage keys total.** All per-browser, all
  ...
  - `umsWhatsNew` — the "What's new" seen-stamp (`{seenStamp}` — the designated
    ...
    panel; ANY dismissal path (Got it, X, Esc, backdrop) stamps it via
...
|-------|----------------------------|------|-------|-----------|----------|
| Time Clock / ADP | `ADP_SS_ID` (CONFIG placeholder) | Employees (roster), Timesheet, TimesheetArchive (cold tier, INV-153), TimeOffRequests, AuditLog, PunchAdjustRequests, ClientErrors (INV-150), SpanishManualResolved | Payroll + shared audit | kept (archive moves, never deletes) | `getAdpSS_` |
