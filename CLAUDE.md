...
- **web-app/** — Multi-module browser web app deployed at one Web App
  ...
  registry reorg; keep it in step with `Object.keys(TOOLS).length`:
   ...
   - **QA** — call-recording review (Phase 1, operator 2026-08-27).
     ...
     patient/agent name, so names stay in the QA store (INV-32). Phase 2
     (waveform, scorecards, per-agent stats) and Phase 3 (sampling,
     calibration, agent-facing reviews) are scoped but unbuilt. See
     INV-196.
...
| QA (recordings) | `QA_SS_ID` (**no fallback**) | QaRecordings (Drive-folder index: status/assignee), QaComments (timestamped review comments — soft-delete, append-only) | QA/HR-adjacent (comments may name patients; reviews reference agents) | kept (no purge tier) | `getQaSS_` |
...
S90 | QA module — sync, queue, playback, timestamped comments | Subsystem: Server, Client (QA views)
  Steps:
    ...
    - In the QA spreadsheet's shared AuditLog rows (`QaSync`/`QaStatusChange`/`QaAssign`/
      `QaCommentAdd` on the ADP sheet's AuditLog tab), confirm counts/ids ONLY — no file
      names, no comment text
