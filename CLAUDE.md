...
- **Self-improving-KB loop — rep freshness signal + content-gap requests
  ...
  **#2 (freshness):** every article/embed reader carries a "Was this helpful?
  ...
  content-authoring tier. Follow-ons: drawer parity (the bar + CTA are Reference-
  tab-only today) and surfacing the 👍/👎 counts (recorded, not yet shown). See
  INV-139.
...
| Knowledge Base + Training | `KB_SS_ID` (CONFIG placeholder) | KB, KbViews, KbFeedback, KbContentRequests, TrainingAssignments, TrainingCompletions, Quizzes, QuizAttempts | PHI-free by policy | kept | `getKbSS_` |
