...
added to both), **plus two cycle-6 tripwires: a trigger-GATE-TYPE check (every
install-`TARGETS` handler calls `assertManagerCaller_` and references no
`.isAdmin` in code — the F1 class) and an Automation-Health label-map check
(client `DIGEST_LABELS` ⊇ server `DIGEST_STALE_HOURS` keys + `CN_HEALTH_RUN_LABELS`
⊇ `AUTOMATION_AUDIT_ACTIONS` — the F5 class)); it also
